...
_PAPER_DESCRIPTIVES = {
    "P-SIS": {
        "HR-G": (4.30, 1.23, 5.0, 0, 5),
        "PC-G": (0.91, 1.54, 0.0, 0, 5),
        "GC-G": (0.44, 1.03, 0.0, 0, 5),
        "Total": (1.37, 1.95, 0.0, 0, 5),
    },
    "MFQ": {
        "HR-G": (40.28, 14.46, 44.0, 7, 63),
        "PC-G": (19.69, 15.47, 15.0, 0, 57),
        "GC-G": (13.80, 11.98, 10.0, 0, 49),
        "Total": (20.86, 16.91, 16.0, 0, 63),
    },
    "SDQ": {
        "HR-G": (19.16, 6.70, 19.0, 1, 34),
        "PC-G": (13.24, 6.27, 14.0, 0, 26),
        "GC-G": (9.33, 5.67, 9.0, 0, 27),
        "Total": (12.41, 7.14, 12.0, 0, 34),
    },
    "EBIP": {
        "HR-G": (10.49, 6.37, 10.0, 0, 29),
        "PC-G": (7.80, 9.04, 4.0, 0, 37),
        "GC-G": (4.86, 4.50, 4.0, 0, 19),
    ...
    "PIUS-a": {
        ...
        "PC-G": (12.93, 7.97, 14.0, 0, 33),
        "GC-G": (12.58, 7.58, 12.0, 0, 31),
        "Total": (13.72, 8.07, 13.0, 0, 36),
    },
}

# Group code → label used in Table 5; `None` selects the whole sample.
_TABLE5_GROUPS = [(1, "HR-G"), (2, "PC-G"), (3, "GC-G"), (None, "Total")]
...
def reproduce(
    ...
    for grp, paper in _PAPER_DEMOGRAPHICS.items():
        ...
        _verify(
            "  Age — std  (trunc)", _trunc(sub["age"].std(ddof=1), 2), paper["age_std"], failures
        )
        _verify(
            "  Education — Primary", int((sub["education"] == 1).sum()), paper["prim"], failures
        )
        _verify(
            "  Education — Secondary", int((sub["education"] == 2).sum()), paper["sec"], failures
        )
        _verify(
            "  Education — Upper Sec.",
            int((sub["education"] == 3).sum()),
            paper["upper"],
            failures,
        )
    ...
    _verify(
        "  Education — Primary   (N)", int((df["education"] == 1).sum()), p["prim_n"], failures
    )
    _verify(
        "  Education — Primary   (%)",
        round((df["education"] == 1).mean() * 100, 2),
        p["prim_pct"],
        failures,
    )
    ...
    _verify(
        "  Education — Secondary (%)",
        round((df["education"] == 2).mean() * 100, 2),
        p["sec_pct"],
        failures,
    )
    _verify(
        "  Education — Upper Sec.(N)", int((df["education"] == 3).sum()), p["upper_n"], failures
    )
    _verify(
        "  Education — Upper Sec.(%)",
        round((df["education"] == 3).mean() * 100, 2),
        p["upper_pct"],
        failures,
    )
    ...
    print(f"\n{'═' * 68}")
    print("  TABLE 5 — Descriptive Statistics by Clinical Group (N=207)")
    print(f"{'═' * 68}")

    for instrument, score_col in SCORE_COLUMNS.items():
        print(f"\n  {instrument}  ({score_col})")
        print(f"  {_sep}")
        for grp, label in _TABLE5_GROUPS:
            data = (df if grp is None else df[df["clinical_group"] == grp])[score_col].dropna()
            paper_mean, paper_sd, paper_median, paper_min, paper_max = _PAPER_DESCRIPTIVES[
                instrument
            ][label]
            _verify(
                f"  {label:<5} mean", round(float(data.mean()), 2), paper_mean, failures, fmt=".2f"
            )
            _verify(
                f"  {label:<5} sd",
                round(float(data.std(ddof=1)), 2),
                paper_sd,
                failures,
                fmt=".2f",
            )
            _verify(
                f"  {label:<5} median",
                round(float(data.median()), 1),
                paper_median,
                failures,
                fmt=".1f",
            )
            _verify(f"  {label:<5} min", int(data.min()), paper_min, failures)
            _verify(f"  {label:<5} max", int(data.max()), paper_max, failures)
